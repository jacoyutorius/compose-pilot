...
- サービスカードからの個別ログ追跡
- `--no-cache`と複数の`--build-arg`を指定できるビルド設定
...
各サービスカードの「ログを追跡」アイコンを押すと、そのサービスの直近200行を2秒間隔で更新します。「実行結果」見出し横の「ログを追跡」アイコンは、Compose Pilot自身を除く全サービスのログをまとめて表示します。追跡中は同じ位置に表示される停止アイコンで終了できます。

各サービスカードの「ビルド設定」アイコンから、そのサービス専用の`--no-cache`と`--build-arg KEY=VALUE`を指定できます。build-argは1行に1つ入力し、「ビルド」と「ビルドして起動」の両方へ適用されます。複数サービスを対象にした場合は、それぞれの設定でサービス単位にbuildを実行します。「ビルドして起動」では、すべてのbuildが成功した後に`docker compose up -d`を実行します。build-argはイメージ履歴などへ残る可能性があるため、パスワードやトークンなどの機密情報には使用しないでください。
