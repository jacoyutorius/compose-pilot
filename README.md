...
「ビルド設定」アイコンから、`--no-cache`と`--build-arg KEY=VALUE`を指定できます。build-argは1行に1つ入力し、「ビルド」と「ビルドして起動」の両方へ適用されます。「ビルドして起動」では、オプション付きの`docker compose build`が成功した後に`docker compose up -d`を実行します。build-argはイメージ履歴などへ残る可能性があるため、パスワードやトークンなどの機密情報には使用しないでください。
