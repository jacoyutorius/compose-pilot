...
操作ボタンは画面の密度を抑えるためアイコンを基本とし、マウス操作とキーボード操作のどちらでもツールチップを表示します。アイコンだけに意味を依存せず、各操作にはアクセシブルな名前を設定します。ブランドロゴとファビコンは外部配信に依存しないSVGを使用します。

配色はライトテーマとダークテーマを用意します。初回表示ではOSのカラースキームを尊重し、利用者が画面上で切り替えた選択はブラウザへ保存します。

デスクトップ幅ではサービス一覧を左側へ縦に並べ、実行結果を右側へ配置します。狭い画面ではサービス一覧と実行結果を上下に並べ、操作領域の幅を確保します。
...
- サービスカードからの個別ログ追跡
- サービスごとに`--no-cache`と複数の`--build-arg`を指定できるビルド設定
- サービス状態の表示
- Composeログの追跡
