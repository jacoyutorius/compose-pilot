...
- `cp .env.example .env`: ローカル設定を作成し、`HOST_PROJECT_ROOT` に管理対象プロジェクトの macOS 上の絶対パスを指定します。
...
`.env`、認証情報、生成された override ファイルはコミットしません。Docker ソケットをマウントするため、公開先は `127.0.0.1` に限定します。元の Compose ファイルを変更せず、`PROJECT_ROOT` 外へのアクセス、Compose Pilot 自身の意図しない操作、危険な多重実行を防いでください。変更完了前にテストを実行し、必要に応じて README と設計文書を更新します。
